...
namespace TMPro
{
    ...
    public abstract class TMP_Text : MaskableGraphic
    {
        ...
        protected bool ValidateHtmlTag(UnicodeChar[] chars, int startIndex, out int endIndex)
        {
            ...
            string tag = new string(m_htmlTag);
            ...
            {
                float value = 0;
                string hText = new string(m_htmlTag);


                Dictionary<string, int> tagsHashcode = new Dictionary<string, int>();
                tagsHashcode.Add("<p>", 112);
                tagsHashcode.Add("<h1>", 777);
                tagsHashcode.Add("<h2>", 778);



                if (m_xmlAttribute[0].nameHashCode == tagsHashcode["<p>"] || m_xmlAttribute[0].nameHashCode == tagsHashcode["<h1>"] || m_xmlAttribute[0].nameHashCode == tagsHashcode["<h2>"])
                {

                    if (hText.Contains("text-align: "))
                    {
                        if (hText.Contains("center"))
                        {
                            m_lineJustification = HorizontalAlignmentOptions.Center;
                            m_lineJustificationStack.Add(m_lineJustification);
                            return true;
                        }
                        else if (hText.Contains("right"))
                        {
                        ...
                        else if (hText.Contains("justified"))
                        {
                            m_lineJustification = HorizontalAlignmentOptions.Justified;
                            m_lineJustificationStack.Add(m_lineJustification);
                        }
                        else if (hText.Contains("flush"))
                        {
                ...
                switch (m_xmlAttribute[0].nameHashCode)
                {
                    ...
                    case 45073: // <samp>
                        ...
                        int location = tag.IndexOf("pt");
                        float size = m_currentFontSize;
                        string n = tag.Substring(location - 2, 2);
                        float.TryParse(n, out size);

                        value = size / (float)initialsize;
                    ...
                    case 45722: //<span>
                        tag = new string(m_htmlTag);

                        if (tag.ToLower().Contains("color:"))
                        {
                            if (tag.Contains("#"))
                            {
                                int colorLocation = tag.IndexOf("#");
                                string colorCode = tag.Substring(colorLocation, 7);
                                Color c = Color.black;
